...
m.setObjective(-0.5*P1-0.5*P2 -12*P3-8*P4-5*P5-5*P6-1*P7-3*P8-2*P9
               -5*Y1-6*Y2-4*Y3-4*Y4-8*Y5-6*Y6-7*Y7
               +0.28*X1+0.3*X2+0.25*X3+0.17*X4+0.31*X5+0.246*X6+0.4*X7, GRB.MAXIMIZE)   

#drug is included or not constraints
c1 = m.addConstr(Y1>=1)
c2 = m.addConstr(Y2<=0)
c3 = m.addConstr(Y3>=1)
c4 = m.addConstr(Y4>=1)
...
c32 = m.addConstr(X1+X2+X3+X4+X5+X6+X7<=100)
c33 = m.addConstr(Y1+Y2+Y3+Y4+Y5+Y6+Y7>=1)

#objective function treshold
